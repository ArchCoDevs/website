...
import { NewsArticle } from 'lib/types/sanity.types';
...
export const getStaticProps: GetStaticProps<PageProps> = async ({
  ...
  const data = await sanityQuery({
    query: `"pageData": *[_type == 'newsArticle' && slug.current == $slug][0]`,
    params: { slug },
    preview
  });
  ...
  return (
    <>
      ...
      >
        <Breadcrumbs
          breadcrumbs={[
            { label: 'News and Reports', href: '/news-and-reports' },
            { label: title, href: `/news-and-reports/${pageData.slug.current}` }
          ]}
        />
        <Main className={styles['news-page']}>
          <Padding>
            ...
            <p className={styles.date}>
              {formatDate(new Date(theDate), 'do MMMM, yyyy')}
            </p>
