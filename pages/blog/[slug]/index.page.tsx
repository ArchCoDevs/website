...
import { BlogArticle } from 'lib/types/sanity.types';
...
export const getStaticProps: GetStaticProps<PageProps> = async ({
  ...
  const data = await sanityQuery({
    query: `"pageData": *[_type == 'blogArticle' && slug.current == $slug][0]`,
    params: { slug },
    preview
  });
...
export const Page = ({ pageData, globals, navigation, preview }: PageProps) => {
  ...
  const { metadata, title, date, summary, heroImage, content, sideContent } =
    pageData;
      ...
      >
        <Breadcrumbs
          breadcrumbs={[
            ...
            { label: title, href: `/blog/${pageData.slug.current}` }
          ]}
        />
