...
If you are running `pnpm run devkit`, the icon will automatically be generated and added to the gallery. Otherwise, you can force the icon generation by running `pnpm run gen:icons`.

<Tip type="warning">
  On occasion, the devkit may fail to generate icons or will cause an error in
  Storybook. If this happens, try running `pnpm run gen:icons` manually and
