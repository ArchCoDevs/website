...
<Tip>
  All skeletons in the storybook stories have `isLoading` set to `true`, with
  the exception of the 'toggle loading' story. Use the controls to see the
  skeleton in its different states in the other stories.
</Tip>
...
<Tip note="note">
  This component is used to provide you with the tools to create skeletons for
  your components. There is also a `skeleton()` mixin that you can use to create
  skeletons for your components (see the 'making a skeleton section below').
</Tip>
...
<Tip type="warning">
  You may have to add a bit of custom styling to a skeleton to prevent content
  from jumping around when the skeleton is toggled on and off. You can use the
  `className` prop to add your own css to any skeleton
</Tip>
