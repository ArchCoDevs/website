...
<Tip type="danger">
  It is very important that you pass the same `id` prop value to the Label and
  the input
</Tip>
...
<Tip type="warning">
  This is not a recommended pattern and should only be used in exceptional
  circumstances. You will also need to manually add any margins to ensure the
  label is positioned correctly.
</Tip>
...
<Tip>
  {' '}
  Some components can have issues when rendered via the InputFactory component. If
  you are having an issue try passing the `parent` prop to the label. This will prevent
  the `htmlFor` attribute from being set which usually fixes the issue.{' '}
</Tip>
